...
class ReasonModal(ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(description=f"**<:roledranzer:1381158073262932028> Target User:** [{self.user}](https://discord.com/users/{self.user.id})\n<:icon_mention:1381315815290179765> **User Mention:** {self.user.mention}\n**<:rp_arrow:1383632907267997716> DM Sent:** {dm_status}\n**<:Vanity_Roles:1383631102887067688> Reason:** {reason}", color=0x000000)
        ...
        embed.add_field(name="<:Automod:1381152776008106014> Moderator:", value=interaction.user.mention, inline=False)
...
class Mute(commands.Cog):
    ...
    async def mute(self, ctx, user: discord.Member, time: str = None, *, reason=None):
        ...
        try:
            await user.send(f"<a:olympus_WarnFlash:1380915318658174976> You have been muted in **{ctx.guild.name}** by **{ctx.author}** for {duration_text}. Reason: {reason or 'None'}")
            dm_status = "Yes"
        except discord.Forbidden:
            dm_status = "No"
        except discord.HTTPException:
        ...
        embed = discord.Embed(description=f"**<:roledranzer:1381158073262932028> Target User:** [{user}](https://discord.com/users/{user.id})\n"
                                          f"<:icon_mention:1381315815290179765> **User Mention:** {user.mention}\n"
                                          f"<:Icon_Mail:1381572449614823425> **DM Sent:** {dm_status}\n"
                                          f"**<:Vanity_Roles:1383631102887067688> Reason:** {reason or 'None'}\n"
                                          f"**<:rp_arrow:1383632907267997716> Duration:** {duration_text}",
                              color=self.color)
        embed.set_author(name=f"Successfully Muted {user.name}", icon_url=self.get_user_avatar(user))
        embed.add_field(name="<:Automod:1383625060652417024> Moderator:", value=ctx.author.mention, inline=False)
        embed.set_footer(text=f"Requested by {ctx.author}", icon_url=self.get_user_avatar(ctx.author))
        embed.timestamp = discord.utils.utcnow()
    ...
    async def mute_error(self, ctx, error):
        ...
        if isinstance(error, commands.BotMissingPermissions):
            embed = discord.Embed(title="<:icon_cross:1381448030481547315> Access Denied", description="I don't have permission to mute members.", color=self.color)
            await ctx.send(embed=embed)
        elif isinstance(error, discord.Forbidden):
            embed = discord.Embed(title="<:icon_cross:1381448030481547315> Missing Permissions", description="I can't mute this user as they might have higher privileges (e.g., Admin).", color=self.color)
            await ctx.send(embed=embed)
            
        else:
            embed = discord.Embed(title="<:icon_cross:1381448030481547315> Unexpected Error", description=str(error), color=self.color)
            await ctx.send(embed=embed)
