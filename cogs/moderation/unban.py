...
class ReasonModal(ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            await self.user.send(f"<a:olympus_WarnFlash:1380915318658174976> You have been Banned from **{self.author.guild.name}** by **{self.author}**. Reason: {reason or 'No reason provided'}")
            dm_status = "Yes"
        ...
        embed.add_field(name="<:Automod:1383625060652417024> Moderator:", value=interaction.user.mention, inline=False)
...
class Unban(commands.Cog):
    ...
    async def unban(self, ctx, user: discord.User, *, reason=None):
        ...
        try:
            await user.send(f"<:IconTick:1381245157759782975> You have been unbanned from **{ctx.guild.name}** by **{ctx.author}**. Reason: {reason or 'No reason provided'}")
            dm_status = "Yes"
        except discord.Forbidden:
            dm_status = "No"
        except discord.HTTPException:
        ...
        embed = discord.Embed(description=f"**<:roledranzer:1381158073262932028> Target User:** [{user}](https://discord.com/users/{user.id})\n**<:icon_mention:1381315815290179765> User Mention:** {user.mention}\n**<:Icon_Mail:1381572449614823425> DM Sent:** {dm_status}\n**<:rp_arrow:1383632907267997716> Reason:** {reasonn}", color=self.color)
