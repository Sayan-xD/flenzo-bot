...
class MuteReasonModal(ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            await self.user.send(f"<:icon_warn:1388333551065501777> You have been muted in **{interaction.guild.name}** for {time_str}. Reason: {reason}")
            dm_status = "Yes"
        except discord.Forbidden:
            dm_status = "No"
        ...
        success_embed = discord.Embed(
            description=f"**<:roledranzer:1381158073262932028> Target User:** [{self.user}](https://discord.com/users/{self.user.id})\n<:icon_mention:1381315815290179765> **User Mention:** {self.user.mention}\n**<:Vanity_Roles:1383631102887067688> Reason:** {reason}\n<:Icon_Mail:1381572449614823425> **DM Sent:** {dm_status}", 
        ...
        success_embed.add_field(name="<:MekoBan:1325354981301293156> Moderator:", value=self.author.mention, inline=False)
...
class Unmute(commands.Cog):
    ...
    async def unmute(self, ctx, user: discord.Member):
        ...
        try:
            ...
            try:
                await user.send(f"<:IconTick:1381245157759782975> You have been unmuted in **{ctx.guild.name}**.")
                dm_status = "Yes"
            except discord.Forbidden:
                dm_status = "No"
            except discord.HTTPException:
        ...
        embed = discord.Embed(
            description=f"**<:roledranzer:1381158073262932028> Target User:** [{user}](https://discord.com/users/{user.id})\n**<:icon_mention:1381315815290179765> **User Mention:** {user.mention}\n<:Icon_Mail:1381572449614823425> **DM Sent:** {dm_status}",
            color=self.color
        )
        embed.set_author(name=f"Successfully Unmuted {user.name}", icon_url=self.get_user_avatar(user))
        embed.add_field(name="<:Automod:1381152776008106014> Moderator:", value=ctx.author.mention, inline=False)
        embed.set_footer(text=f"Requested by {ctx.author}", icon_url=self.get_user_avatar(ctx.author))
