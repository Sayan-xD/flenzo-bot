...
class OnOrOff(BasicView):
    ...
    @discord.ui.button(label="DM me", emoji="<:IconTick:1381245157759782975>", custom_id='Yes', style=discord.ButtonStyle.green)
    async def dare(self, interaction: discord.Interaction, button: discord.ui.Button):
        self.value = 'Yes'
        await interaction.response.defer()
        self.stop()

    @discord.ui.button(label="Don't DM", emoji="<:icon_cross:1381448030481547315>", custom_id='No', style=discord.ButtonStyle.danger)
...
class afk(commands.Cog):
    ...
    async def afk(self, ctx, *, reason=None):
        ...
        if any(invite in reason.lower() for invite in ['discord.gg', 'gg/']):
            emd = discord.Embed(
                description="<:icon_cross:1381448030481547315> | You can't advertise Server Invite in the AFK reason",
                color=0x0c0606
            )
            return await ctx.send(embed=emd)

        async with aiosqlite.connect(DB_PATH) as db:
            dm_status = 'True'  # always DM
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            ...
            await test.delete()
            af = discord.Embed(title='<:IconTick:1381245157759782975> Success', 
                 description=f'{ctx.author.mention}, You are now marked as AFK due to: **{reason}**', 
                 color=0x000000)
            await ctx.reply(embed=af)
