...
class TimeSelect(Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        guild = interaction.client.get_guild(968855422423404606)
        if guild:
            member = guild.get_member(self.user.id)
            if member:
                role = guild.get_role(1395964329915846656)
                if role:
                    await member.add_roles(role, reason="No prefix added")

            

        log_channel = interaction.client.get_channel(1388289702645203034)
        if log_channel:
            embed = discord.Embed(
                title="User Added to No Prefix",
                description=f"**<:roledranzer:1381158073262932028> User**: [{self.user}](https://discord.com/users/{self.user.id})\n**<:icon_mention:1381315815290179765> User Mention**: {self.user.mention}\n**<:icons_id:1381317053172154511> ID**: {self.user.id}\n\n**<:Automod:1383625060652417024> Added By**: [{self.author.display_name}](https://discord.com/users/{self.author.id})\n<:Time_Icon:1381318601885352036> **Expiry Time**: {expiry_text}\n<:rp_arrow:1383632907267997716> **Timestamp**: {expiry_timestamp}\n\n<a:iconodiamante:1381319372605362399> **Tier**: **{self.values[0].upper()}**",
                color=0x000000
            )
            embed.set_thumbnail(url=self.user.avatar.url if self.user.avatar else self.user.default_avatar.url)
            await log_channel.send("<@833509477754994698>, <@961805595449638952>, <@931499515410800690>",embed=embed)
            

        
        embed = discord.Embed(description=f"**__Successfully Added No Prefix To {self.user.mention} For {self.values[0].upper()}__**", color=0x000000)
        embed.set_author(name="Success", icon_url="https://cdn.discordapp.com/emojis/1381245157759782975.png")
...
class NoPrefix(commands.Cog):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            if expired_users:
                ...
                for user_id in expired_users:
                    ...
                    if user:
                        log_channel = self.client.get_channel(1388289702645203034)
                        if log_channel:
                            embed_log = discord.Embed(
                                title="No Prefix Expired",
                                description=(
                                    f"**<:roledranzer:1381158073262932028> User**: [{user}](https://discord.com/users/{user.id})\n"
                                    f"**<:icon_mention:1381315815290179765> User Mention**: {user.mention}\n"
                                    f"**<:icons_id:1381317053172154511> ID**: {user.id}\n\n"
                                    f"**<:rp_arrow:1383632907267997716> Removed By**: **[Flenzo](https://discord.com/users/998547879201349693)**\n"
                                ),
                                color=0x000000
                            )
                            ...
                            await log_channel.send("<@833509477754994698>, <@961805595449638952>, <@931499515410800690>", embed=embed_log)
                        bot = self.client
                        guild = bot.get_guild(968855422423404606)
                        if guild:
                        ...
                        embed = discord.Embed(
                            description=f"<:icon_cross:1381448030481547315> Your No Prefix status has **Expired**. You will now require the prefix to use commands.",
                            color=0x000000
                        )
                        embed.set_author(name="No Prefix Expired", icon_url=user.avatar.url if user.avatar else user.default_avatar.url)
                        
                        embed.set_footer(text="Flenzo - No Prefix, Join support to regain access.")
    ...
    async def np_add(self, ctx, user: discord.User):
        async with aiosqlite.connect(self.db_path) as db:
            ...
            if result:
                embed = discord.Embed(description=f"**{user}** is Already in No prefix list\n\n<:Vanity_Roles:1383631102887067688> **Requested By**: [{ctx.author.display_name}](https://discord.com/users/{ctx.author.id})\n", color=0x000000)
    ...
    async def np_remove(self, ctx, user: discord.User):
        ...
        guild = ctx.bot.get_guild(968855422423404606)
        if guild:
            member = guild.get_member(user.id)
            if member:
                role = guild.get_role(1395964329915846656)
        ...
        embed = discord.Embed(
                description=(
                    f"**<:roledranzer:1381158073262932028> User**: [{user}](https://discord.com/users/{user.id})\n"
                    f"**<:icon_mention:1381315815290179765> User Mention**: {user.mention}\n"
                    f"**<:icons_id:1381317053172154511> User ID**: {user.id}\n\n"
                    f"**<:Automod:1383625060652417024> Removed By**: [{ctx.author.display_name}](https://discord.com/users/{ctx.author.id})\n"
                ),
            color=0x000000
        )
        ...
        log_channel = ctx.bot.get_channel(1388289702645203034)
        if log_channel:
            embed_log = discord.Embed(
            ...
            embed_log.set_thumbnail(url=user.display_avatar.url if user.avatar else user.default_avatar.url)
            embed_log.set_footer(text="No Prefix Removal Log")
            await log_channel.send("<@961805595449638952>, <@833509477754994698>, <@931499515410800690>", embed=embed_log)
        ...
        async with aiosqlite.connect('db/np.db') as db:
            ...
            if not result:
                embed = discord.Embed(
                    ...
                    description=f"**{user}** is Not in the No Prefix list\n\n"
                                f"<:Automod:1383625060652417024> **Requested By**: "
            ...
            embed = discord.Embed(
                ...
                description=(
                    f"**<:roledranzer:1381158073262932028> User**: [{user}](https://discord.com/users/{user.id})\n"
                    f"**<:icons_id:1381317053172154511> User ID**: {user.id}\n\n"
                    f"**<:Time_Icon:1381318601885352036> Expiry**: {expire_time} ({expire_timestamp})"
                ),
                color=0x000000
            )
    ...
    async def on_member_update(self, before, after):
        if before.premium_since is None and after.premium_since is not None:
            ...
            if not await self.is_user_in_np(after.id):
                await self.add_np(after, timedelta(days=30))
                log_channel = self.client.get_channel(1388289702645203034)
                embed = discord.Embed(
                    title="Added No prefix due to Boosting Our Server",
                    description=f"**User**: **[{after}](https://discord.com/users/{after.id})** (ID: {after.id})\n**Server**: {after.guild.name}",
                    color=0x00FF00
                )
                message = await log_channel.send("<@833509477754994698>, <@961805595449638952>, <@931499515410800690>", embed=embed)
    ...
    async def handle_boost_removal(self, user):
        ...
        if await self.is_user_in_np(user.id):
            ...
            log_channel = self.client.get_channel(1388289702645203034)
            embed = discord.Embed(
                title="Removed No prefix due to Unboosting Our Server",
    ...
    async def add_np(self, user, duration):
        ...
        embed = discord.Embed(
                            title=" Congratulations you got 1 months No Prefix!",
                            description=f"<a:gift2:1392786541931135018> You've been credited 1 months of global No Prefix for boosting our Our Server. You can now use my commands without prefix. If you wish to remove it, please reach out [Support Server](https://discord.gg/C86nD33WBr).",
                            color=0x000000
                        )
        try:
        ...
        guild = self.client.get_guild(968855422423404606)
        if guild:
            member = guild.get_member(user.id)
    ...
    async def remove_np(self, user):
        ...
        embed= discord.Embed(title="Global No Prefix Expired",
                        description=f"Hey {user.mention}, your global no prefix has expired!\n\n__**Reason:**__ Unboosting our Server.\nIf you think this is a mistake then please reach out [Support Server](https://discord.gg/C86nD33WBr).",
                        color=0x000000)
