...
class MusicControlView(View):
    ...
    async def pause_button(self, interaction: discord.Interaction, button: Button):
        if self.player.paused:
            ...
            await self.player.channel.edit(status=f"<:gvMusic:1321046882167947324> Playing: {self.player.current.title}")
            button.emoji = "<:o_pause:1302172935951351821>" 
            await interaction.response.edit_message(view=self)

        elif self.player.playing:
            await self.player.pause(True)
            await self.player.channel.edit(status=f"<:gvMusic:1321046882167947324> Paused: {self.player.current.title}")
...
class Music(commands.Cog):
    ...
    async def inactivity_timer(self, guild):
        ...
        if len(guild.voice_channels[0].members) == 1:
            ...
            if player:
                ...
                try:
                    ended = discord.Embed(description="Bot has been disconnected due to inactivity (being idle in Voice Channel)" , color=0xFF0000)
                    ended.set_author(name="Inactive Timeout", icon_url=self.client.user.avatar.url)
                    ended.set_footer(text="Thanks for choosing Flenzo")
                    support = Button(label='Support',
                                 style=discord.ButtonStyle.link,
                        url=f'https://dsc.gg/flenzobot')
                    vote = Button(label='Invite Me',
                                 style=discord.ButtonStyle.link,
                        url=f'https://dsc.gg/addflenzo')
                    view = View()
    ...
    async def connect_nodes(self) -> None:
        nodes = [wavelink.Node(uri="http://pnode1.danbot.host:1186/",   password="Yuvraj.apk#001")]
    ...
    async def on_track_end(self, payload: wavelink.TrackEndEventPayload):
        ...
        if not player.queue:
            if player.queue.mode == wavelink.QueueMode.loop:
                ...
                pass
                ended = discord.Embed(description="All tracks have been played, play another song" , color=0xFF0000)
    ...
    async def play_source(self, ctx, query):
        if not ctx.author.voice:
            await ctx.send(embed=discord.Embed(description="<:icon_cross:1381448030481547315> You need to be in a voice channel to use this command.", color=0x000000))
            return
        ...
        if isinstance(tracks, wavelink.Playlist):
            ...
            await ctx.send(embed=discord.Embed(description=f"<:add_white:1381239415522852916> Added playlist [{tracks.name}](https://dsc.gg/flenzobot) with **{len(tracks.tracks)} songs** to the queue.", color=0x000000))
            if not vc.playing:
            ...
            track = tracks[0]
            await vc.queue.put_wait(track)
            await ctx.send(embed=discord.Embed(description=f"<:add_white:1381239415522852916> Added [{track.title}](https://dsc.gg/flenzobot) to the queue.", color=0x000000))
            if not vc.playing:
            ...
            if type_ == "track":
                ...
                for track in tracks:
                    ...
                    if track_results:
                        ...
                        c += 1
                        await ctx.message.add_reaction("<:IconTick:1381245157759782975>")

                await ctx.send(embed=discord.Embed(description=f"<:add_white:1381239415522852916> Added **{c}** of **{playlist_length}** tracks from **playlist** **[{playlist_info['name']}](https://dsc.gg/flenzobot)** to the queue.", color=0x000000))
                await lmao.delete()
                ...
                await ctx.send(embed=discord.Embed(description=f"<:add_white:1381239415522852916> Added all tracks from album **[{album_info['name']}](https://discord.gg/odx)** to the queue.", color=0x000000))
                if not vc.playing:
                    next_track = await vc.queue.get_wait()
                    await vc.play(next_track)
                    await self.display_player_embed(vc, next_track, ctx)
    ...
    async def autoplay(self, ctx: commands.Context):
        vc = ctx.voice_client
        if not vc or not vc.playing:
            await ctx.send(embed=discord.Embed(description="<:icon_cross:1381448030481547315> No song is currently playing.", color=0x000000))
            return

        if not ctx.author.voice or ctx.author.voice.channel.id != vc.channel.id:
            await ctx.send(embed=discord.Embed(description="<:icon_cross:1381448030481547315> You need to be in the same voice channel as me to use this command.", color=0xFF0000))
            return
        ...
        if vc:
            ...
            await ctx.send(embed=discord.Embed(description=f"<:IconTick:1381245157759782975>  Autoplay {'enabled' if vc.autoplay == wavelink.AutoPlayMode.enabled else 'disabled'} by {ctx.author.mention}.", color=0x000000))
    ...
    async def loop(self, ctx: commands.Context):
        ...
        if vc:
            ...
            await ctx.send(embed=discord.Embed(description=f"<:IconTick:1381245157759782975>  Loop {'enabled' if vc.queue.mode == wavelink.QueueMode.loop else 'disabled'} by {ctx.author.mention}.", color=0x000000))
        else:
    ...
    async def pause(self, ctx: commands.Context):
        ...
        if vc and vc.playing and not vc.paused:
            await vc.pause(True)
            await vc.channel.edit(status=f"<:gvMusic:1321046882167947324> Paused: {vc.current.title}")
            await ctx.send(embed=discord.Embed(description=f"Paused by {ctx.author.mention}.", color=0x000000))
        else:
            await ctx.send(embed=discord.Embed(description="<:icon_cross:1381448030481547315> Nothing is playing or already paused.", color=0xFF0000))
    ...
    async def resume(self, ctx: commands.Context):
        ...
        if vc and vc.paused:
            ...
            await vc.channel.edit(status=f"<:gvMusic:1321046882167947324> Playing: {vc.current.title}")
    ...
    async def skip(self, ctx: commands.Context):
        ...
        if vc and vc.playing and not vc.queue.is_empty:
            ...
            await ctx.send(embed=discord.Embed(description="<:icon_cross:1381448030481547315> No song is playing or in the queue to skip.", color=0xFF0000))
    ...
    async def volume(self, ctx: commands.Context, level: int):
        ...
        if not vc:
            await ctx.send(embed=discord.Embed(description="<:icon_cross:1381448030481547315> I'm not connected to a voice channel.", color=0xFF0000))
            return
        ...
        if vc:
            if 1 <= level <= 150:
                ...
                await ctx.send(embed=discord.Embed(description=f"<:IconTick:1381245157759782975> Volume set to {level}% by {ctx.author.mention}.", color=0x000000))
            else:
                await ctx.send(embed=discord.Embed(description="<:icon_cross:1381448030481547315> Volume must be between 1 and 150.", color=0xFF0000))
        else:
    ...
    async def queue(self, ctx: commands.Context):
        ...
        if not vc or not vc.queue or vc.queue.is_empty:
            await ctx.send(embed=discord.Embed(description="<:icon_cross:1381448030481547315> The queue is currently empty.", color=0x000000))
            return
    ...
    async def clearqueue(self, ctx: commands.Context):
        ...
        if not vc or not vc.queue or vc.queue.is_empty:
            await ctx.send(embed=discord.Embed(description="<:icon_cross:1381448030481547315> No Queue to clear.", color=0xFF0000))
            return
    ...
    async def replay(self, ctx: commands.Context):
        ...
        if not vc or not vc.playing:
            await ctx.send(embed=discord.Embed(description="<:icon_cross:1381448030481547315> I'm not connected to any voice channel.", color=0xFF0000))
            return
    ...
    async def on_wavelink_track_start(self, payload: wavelink.TrackStartEventPayload):
        ...
        if voice_channel:
            await voice_channel.edit(status=f"<:gvMusic:1321046882167947324> Playing: {track.title}")  # type: ignore
