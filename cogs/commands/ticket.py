...
class tickredel(discord.ui.View):
    ...
    async def _treopen(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if p is not None:
            ...
            message = f"{user.mention} Welcome Back! The ticket is reopened"
        embed = discord.Embed(description="You will be provided with support shortly\nTo close this ticket click the <:lock:1399363563923181642> button.", color=0xc283fe).set_footer(text=f"{guild.me.name} Ticket System", icon_url=guild.me.avatar.url)
...
class ticketchannelpanel(discord.ui.View):
    ...
    @discord.ui.button(label="Close", emoji="<:lock:1399363563923181642>", custom_id="close", style=discord.ButtonStyle.red)
    async def _ticketchannel(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        em = discord.Embed(title="<:lock:1399363563923181642> Ticket Closed", description=f"{interaction.channel.mention} is closed by {str(interaction.user)}", color=0xc283fe).set_footer(text=f"{guild.me.name} Ticket System", icon_url=guild.me.avatar.url)
