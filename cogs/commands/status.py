...
class Status(commands.Cog):
    ...
    async def status(self, ctx, user: discord.User = None):
        ...
        processing = await ctx.send("<a:MekoColoredLoading:1378753987515252746> Loading Status...")
        embed = discord.Embed(title=f"{user.display_name}'s Status", color=0x000000)

        status_emoji = {
            "online": "<:online:1378754332757065758> Online",
            "idle": "<:s_Idle:1378754335814586389> Idle",
            "dnd": "<:dnd:1378754341120507926> Do Not Disturb",
            "offline": "<:offline:1378754347206447257> Offline"
        }
        ...
        if member:
            status = status_emoji.get(str(member.status), "<:offline:1378754347206447257> Offline")
            ...
            try:
                ...
                embed.add_field(name="Status:", value="<:offline:1378754347206447257> Offline", inline=False)
    ...
    def get_platform(self, member):
        if member.desktop_status != discord.Status.offline:
            return "<:pc:1324035332731043871> Desktop"
        elif member.mobile_status != discord.Status.offline:
            return "<:mobile:1324035310157299753> Mobile"
        elif member.web_status != discord.Status.offline:
            return "<:browser:1324035353543315467> Browser"
        return "Unknown"
