...
class Nightmode(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.ricky = ['1149183535505227887']
        self.color = 0x000000  
    ...
    async def nightmode(self, ctx):
        ...
        nightmode_embed.add_field(
            name="Usage",
            value="<:red_dot:1317435211453300848>> `nightmode enable`\n<:red_dot:1317435211453300848>> `nightmode disable`",
            inline=False
        )
    ...
    async def enable_nightmode(self, ctx):
        if ctx.guild.member_count < 50:  
            return await ctx.send(embed=discord.Embed(title="<:x_cross:1317432060671037461>> Access Denied",
                color=self.color,
                description='Your Server Doesnt Meet My 50 Member Criteria'
            ))
        ...
        if not manageable_roles:
            return await ctx.send(embed=discord.Embed(title="<:x_cross:1317432060671037461>>  Error",
                color=self.color,
        ...
        await ctx.send(embed=discord.Embed(title="<:tick:1317432275134058506> Success",
            color=self.color,
