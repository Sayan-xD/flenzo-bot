...
class AutoReaction(commands.Cog):
    ...
    async def add(self, ctx, trigger: str, *, emojis: str):
        if len(trigger.split()) > 1:
            embed = discord.Embed(
                title="<:icon_cross:1381448030481547315> Invalid Trigger",
        ...
        if len(emoji_list) > 20:
            embed = discord.Embed(
                title="<:icon_cross:1381448030481547315> Too Many Emojis",
                description="You can only set up to **20** emojis per trigger.",
                color=0x000000
            )
            embed.set_footer(text=f"Requested By {ctx.author}",
                   icon_url=ctx.author.avatar.url if ctx.author.avatar else ctx.author.default_avatar.url)
        ...
        if len(triggers) >= 10:
            embed = discord.Embed(
                title="<:icon_cross:1381448030481547315> Trigger Limit Reached",
        ...
        if await self.trigger_exists(ctx.guild.id, trigger):
            embed = discord.Embed(
                title="<:icon_cross:1381448030481547315> Trigger Exists",
        ...
        embed = discord.Embed(
            title="<:IconTick:1381245157759782975> Trigger Added",
    ...
    async def remove(self, ctx, trigger: str):
        if not await self.trigger_exists(ctx.guild.id, trigger):
            embed = discord.Embed(
                title="<:icon_cross:1381448030481547315> Trigger Not Found",
        ...
        embed = discord.Embed(
            title="<:IconTick:1381245157759782975> Trigger Removed",
    ...
    async def reset(self, ctx):
        ...
        if not triggers:
            embed = discord.Embed(
                title="<:x_cross:1317432060671037461> No Triggers Set",
        ...
        embed = discord.Embed(
            title="<:IconTick:1381245157759782975> All Triggers Reset",
    ...
    async def on_message(self, message):
        ...
        for trigger, emojis in triggers:
            if trigger.lower() in message.content.lower():  # Always contains
