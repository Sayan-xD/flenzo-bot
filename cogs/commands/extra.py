...
tick = "<:IconTick:1381245157759782975>"
cross = "<:icon_cross:1381448030481547315>"
...
class RoleInfoView(View):
  ...
  @discord.ui.button(label='Show Permissions',  emoji="<:roledranzer:1381158073262932028> ", style=discord.ButtonStyle.secondary)
...
class Extra(commands.Cog):
  ...
  async def uptime(self, ctx):
      ...
      embed = discord.Embed(title=f"Flenzo's Uptime", color=self.color)
      embed.add_field(name="__UTC__", value=f"<:Icon_Info:1381446309172744345> {uptime_string}\n\n", inline=False)
      embed.add_field(name="__Online Duration__", value=f"<:icons_online:1381454833873191033>  {uptime_duration_string}", inline=False)
  ...
  async def serverinfo(self, ctx):
        ...
        embed.add_field(
            name="**__About__**",
            value=f"**Name : ** {ctx.guild.name}\n**ID :** {ctx.guild.id}\n**Owner <:gold_owner:1389297086704521226>  :** {ctx.guild.owner} (<@{ctx.guild.owner_id}>)\n**Created At : ** {c_at}\n**Members :** {len(ctx.guild.members)}",
            inline=False
        )
        ...
        if ctx.guild.features:
            features = "\n".join([f"<:IconTick:1381245157759782975>: {feature[:1].upper() + feature[1:].lower().replace('_', ' ')}" for feature in ctx.guild.features])
            embed.add_field(
                name="**__Features__**",
        ...
        embed.add_field(
            ...
            value=f"Level: {ctx.guild.premium_tier} [<:icon_booster:1381451121654104155> {ctx.guild.premium_subscription_count} boosts]",
            inline=False
        )
...
**ID:** {member.id}
**Nickname:** {nickk}
**Bot?:** {'<:IconTick:1381245157759782975> Yes' if member.bot else '<:icon_cross:1381448030481547315> No'}
**Badges:** {badges}
...
**Color:** {member.color if member.color else '99aab5'}
  ...
  async def boosts(self, ctx):
    await ctx.send(
      embed=discord.Embed(title=f"<:icon_booster:1381451121654104155> Boosts Count Of {ctx.guild.name}",
  ...
  async def report(self, ctx, *, bug):
    channel = self.bot.get_channel(1384788248429330462)
    ...
    confirm_embed = discord.Embed(title="<:IconTick:1381245157759782975> Bug Reported",
