...
class Extraowner(commands.Cog):
    ...
    async def extraowner(self, ctx, option: str = None, user: discord.Member = None):
        ...
        if ctx.guild.member_count < 100:
            embed = discord.Embed(
                description="<a:MekoCross:1373319009818116176> | Your Server Doesn't Meet My 100 Member Criteria",
                color=0x000000
            )
            await ctx.send(embed=embed)
            return

        Flenzo = ['833509477754994698']
        if ctx.author.id != ctx.guild.owner_id and str(ctx.author.id) not in Flenzo:
        ...
        if option.lower() == 'set':
            ...
            if view.value is None:
                ...
                await message.edit(content="<:icon_cross:1381448030481547315> Action cancelled.", embed=None, view=None)
