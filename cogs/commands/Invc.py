...
class Invcrole(commands.Cog):
    ...
    @commands.group(name='vcrole', help="Vc roles Setup commands", invoke_without_command=True)
    @blacklist_check()
    @ignore_check()
            ...
            async with aiosqlite.connect(self.db_path) as db:
                async with db.execute('SELECT role_id FROM vcroles WHERE guild_id = ?', (member.guild.id,)) as cursor:
                    ...
                    if after.channel and role not in member.roles:
                        await self.add_role_with_retry(member, role, reason="Member Joined VC | Flenzo Invcrole")
                    elif not after.channel and role in member.roles:
                        await self.remove_role_with_retry(member, role, reason="Member Left VC | Flenzo Invcrole")
