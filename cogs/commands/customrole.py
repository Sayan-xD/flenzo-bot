...
class Customrole(commands.Cog):
    ...
    async def add_role(self, *, role_id: int, member: discord.Member):
        if member.guild.me.guild_permissions.manage_roles:
            ...
            if role is None:
                raise discord.NotFound(f"Role with ID {role_id} not found.")
            await member.add_roles(role, reason="Flenzo Customrole | Role Added")
        else:
            raise discord.Forbidden(member.guild, "Bot does not have permission to manage roles.")
    ...
    async def remove_role(self, *, role_id: int, member: discord.Member):
        if member.guild.me.guild_permissions.manage_roles:
            ...
            if role is None:
                raise discord.NotFound(f"Role with ID {role_id} not found.")
            await member.remove_roles(role, reason="Flenzo Customrole | Role Removed")
        else:
            raise discord.Forbidden(member.guild, "Bot does not have permission to manage roles.")
    ...
    async def add_role2(self, *, role: int, member: discord.Member):
        if member.guild.me.guild_permissions.manage_roles:
            ...
            if role_obj is None:
                raise discord.NotFound(f"Role with ID {role} not found.")
            await member.add_roles(role_obj, reason="Flenzo Customrole | Role Added ")
    ...
    async def remove_role2(self, *, role: int, member: discord.Member):
        if member.guild.me.guild_permissions.manage_roles:
            ...
            if role_obj is None:
                raise discord.NotFound(f"Role with ID {role} not found.")
            await member.remove_roles(role_obj, reason="Flenzo Customrole| Role Removed")
                            ...
                            if role:
                                if role not in member.roles:
                                    try:
                                        ...
                                        embed = discord.Embed(title="<:IconTick:1381245157759782975> Success",
                                            description=f"**Given** <@&{role.id}> To {member.mention}",
                                            color=0x000000)
                                    except discord.Forbidden:
                                        embed = discord.Embed(
                                            title="<:icon_cross:1381448030481547315> Error",
                                            description="I do not have permission to manage this role. Please ensure my role is higher than the target role.\n\nCommand used: `{}`".format(context.message.content),
                                            color=0x000000
                                        )
                                else:
                                    try:
                                ...
                                embed = discord.Embed(title="<:icon_cross:1381448030481547315> Error",
                                    description=f"{role_type.capitalize()} role is not set up in {context.guild.name}",
                                    color=0x000000)
                                await context.reply(embed=embed)
                        else:
                            embed = discord.Embed(title="<:icon_cross:1381448030481547315> Access Denied",
    ...
    async def update_role_data(self, guild_id, column, value):
        try:
            async with aiosqlite.connect(DATABASE_PATH) as db:
                await db.execute(f"UPDATE roles SET {column} = ? WHERE guild_id = ?", (value, guild_id))
                await db.commit()
    ...
    @set.command(name="staff", description="Setup staff role in guild", help="Setup staff role in Guild")
    @commands.cooldown(1, 3, commands.BucketType.user)
    @commands.has_permissions(administrator=True)
    ...
    @set.command(name="girl", description="Setup girl role in the Guild", help="Setup girl role in the Guild")
    @commands.cooldown(1, 3, commands.BucketType.user)
    @commands.has_permissions(administrator=True)
    @app_commands.describe(role="Role to be added")
    ...
    @set.command(name="vip", description="Setups vip role in the Guild", help="Setups vip role in the Guild")
    @commands.cooldown(1, 3, commands.BucketType.user)
    @commands.has_permissions(administrator=True)
    @app_commands.describe(role="Role to be added")
    ...
    @set.command(name="guest", description="Setup guest role in the Guild", help="Setup guest role in the Guild")
    @blacklist_check()
    @ignore_check()
    @commands.cooldown(1, 3, commands.BucketType.user)
    @commands.has_permissions(administrator=True)
    ...
    @set.command(name="friend", description="Setup friend role in the Guild", help="Setup friend role in the Guild")
    @blacklist_check()
    @ignore_check()
    @commands.cooldown(1, 3, commands.BucketType.user)
    @commands.has_permissions(administrator=True)
    ...
    @set.command(name="reqrole", description="Setup required role for custom role commands", help="Setup required role for custom role commands")
    @blacklist_check()
    @ignore_check()
    ...
    @set.command(name="config", description="Shows the current custom role configuration in the Guild.", help="Shows the current custom role configuration in the Guild.")
    @blacklist_check()
    @ignore_check()
    @commands.cooldown(1, 3, commands.BucketType.user)
    @commands.has_permissions(administrator=True)
    ...
    @set.command(name="create", description="Creates a custom role command.", help="Creates a custom role command")
    @blacklist_check()
    @ignore_check()
    ...
    @set.command(name="delete", aliases=["remove"], description="Deletes a custom role command.", help="Deletes a custom role command.")
    @blacklist_check()
    @ignore_check()
    @commands.cooldown(1, 5, commands.BucketType.user)
    @commands.has_permissions(administrator=True)
    ...
    @set.command(name="list", description="List all the custom roles setup for the server.", help="List all the custom roles setup for the server.")
    @blacklist_check()
    @ignore_check()
    @commands.cooldown(1, 3, commands.BucketType.user)
    @commands.has_permissions(administrator=True)
    ...
    @set.command(name="reset", description="Resets custom role configuration for the server.", help="Resets custom role configuration for the server.")
    @blacklist_check()
    @ignore_check()
    @commands.cooldown(1, 4, commands.BucketType.user)
    @commands.has_permissions(administrator=True)
            ...
            if role_data:
                ...
                async with aiosqlite.connect(DATABASE_PATH) as db:
                    ...
                    embed = discord.Embed(
                        ...
                        description=f"Deleted All Custom Role commands <:IconTick:1381245157759782975>\n\n**Removed Roles:**\n" + "\n".join(removed_roles) if removed_roles else "No roles were previously set.",
                        color=0x000000
                    )
                    await context.reply(embed=embed)
            else:
    ...
    async def on_message(self, message: discord.Message):
        ...
        if result:
            ...
            if reqrole is None:
                await message.channel.send("<:icon_cross:1381448030481547315> The required role is not set up in this server. Please set it up using `setup reqrole`.")
                return

            if reqrole not in message.author.roles:
                await message.channel.send(embed=discord.Embed(description=f"<:icon_cross:1381448030481547315>  You need the {reqrole.mention} role to use this command.", color=0x000000))
                return
            ...
            try:
                ...
                await message.channel.send(
                    embed=discord.Embed(
                        title="<:icon_cross:1381448030481547315> Error",
                        description=f"I do not have permission to manage this role. Please ensure my role is higher than the target role.\n\nCommand used: `{message.content}`",
                        color=0x000000
                    )
                )
            except Exception as e:
