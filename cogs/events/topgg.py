...
class TopGG(commands.Cog):
    ...
    async def on_dbl_vote(self, data):
        ...
        embed = discord.Embed(title="<:IconTick:1381245157759782975> [Voted](https://top.gg/bot/998547879201349693/vote)", color=0xff0000)
        next_vote_time = datetime.datetime.now() + datetime.timedelta(hours=12)
        embed.add_field(name="<:Time_Icon:1381318601885352036> Can vote again at:", value=f"<t:{int(next_vote_time.timestamp())}:R>", inline=False)
        embed.add_field(name="<:Utility:1381152992052645890> Total Votes:", value=str(new_count), inline=False)
        embed.add_field(name="<:icons_friends:1381286989126438963> Current Vote Streak:", value=str(new_streak), inline=False)
        embed.timestamp = datetime.datetime.now()
        channel = self.bot.get_channel(1386709333320138843)
