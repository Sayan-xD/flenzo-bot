...
from core import Flenzo, Cog, Context
...
class Errors(Cog):
  ...
  async def on_command_error(self, ctx: Context, error):
    ...
    if isinstance(error, commands.CommandOnCooldown):
      embed = discord.Embed(color=0x000000, description=f"<:icon_cross:1381448030481547315> | {ctx.author.mention} slow down there! You can run the command again in **{error.retry_after:.2f}** seconds.")
    ...
    if isinstance(error, commands.MaxConcurrencyReached):
      embed = discord.Embed(color=0x000000, description=f"<:icon_cross:1381448030481547315> | {ctx.author.mention} This command is already in progress. Please let it finish and try again afterward.")
    ...
    if isinstance(error, commands.MissingPermissions):
      ...
      embed = discord.Embed(color=0x000000, description=f"<:icon_cross:1381448030481547315> | You must have `{fmt}` permissions to use this command.")
    ...
    if isinstance(error, commands.BotMissingPermissions):
      ...
      await ctx.reply(f'<:icon_cross:1381448030481547315> | I need **{missing}** Permission to run the **{ctx.command.qualified_name}** command!', delete_after=7)
      return
